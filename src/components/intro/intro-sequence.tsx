...
import { INVITATION_TEXT_DURATION_MS } from "./invitation-lines";
...
export function IntroSequence({ onComplete, onScrollToForm }: IntroSequenceProps) {
  ...
  return (
    ...
    >
      ...
      <AnimatePresence>
        {showInvitation && <InvitationScene key="scene" showText={showText} />}
      </AnimatePresence>

      {showInvitation && (
        <motion.div
          className="absolute bottom-8 left-1/2 z-20 -translate-x-1/2 text-gold/50"
          initial={{ opacity: 0 }}
